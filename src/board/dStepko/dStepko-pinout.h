...
_MAKE_MOTATE_PIN(kLED_USBRXPinNumber, 'A', 5);           // Socket4_EnablePinNumber
											  ...
											  // pcb net	- real life
_MAKE_MOTATE_PIN(kInput1_PinNumber, 'B', 15); // limitX 	- sensorY
_MAKE_MOTATE_PIN(kInput3_PinNumber, 'B', 14); // limitY		- sensorX
_MAKE_MOTATE_PIN(kInput6_PinNumber, 'B', 13); // limitZ 	- Ne radi
_MAKE_MOTATE_PIN(kInput7_PinNumber, 'B',  8); // limitY2	- sensorZ
_MAKE_MOTATE_PIN(kInput8_PinNumber, 'B',  9); // touchPlate
/*
...
*/
_MAKE_MOTATE_PIN(kSpindle_PwmPinNumber, 'C',  8);



typedef TimerChannel<8,3> parentTimerType;
template<>
struct AvailablePWMOutputPin<ReversePinLookup<'C', 8>::number> : RealPWMOutputPin< ReversePinLookup<'C', 8>::number, parentTimerType>
{
	static const pin_number pinNum = ReversePinLookup<'C', 8>::number;
	AvailablePWMOutputPin() : RealPWMOutputPin<pinNum, parentTimerType>(kOutputPWM)
	{
		pwmpin_init(true ? kPWMOnInverted : kPWMOn);
	};
	...
	static constexpr bool _isAvailable()
	{
		return !ReversePinLookup<'C', 8>::isNull();
	};
};
